...
def comp_next_flow(video_capture, prev_gray, flow_comp, \
        frame_num, bounds, pixrange, save_path, filetype='png', \
        save_img=False):
    """
    ...
    # Compute flow
    flow = flow_comp(prev_gray, gray, None)
...
def flowify(video_file, step, bounds, save_path, pixrange=[0,255]):
    """
    ...
    try:
        ...
        gray = comp_next_flow(cap, first_gray, flow_comp, \
                frame_num, bounds, pixrange, save_path)
        frame_num += 1
        while (gray is not None):
            ...
            gray = comp_next_flow(cap, gray, flow_comp, \
                    frame_num, bounds, pixrange, save_path)
            frame_num += 1
    finally:
...
def execute(vids, step, bound, serial, save_path):
    """
    ...
    if serial:
        for v in vids:
            flowify(v, step, [-bound, bound], save_path)
    else:
...
def main(args):
    ...
    execute(vl, args.frame_step, args.bound, args.serial, \
           args.output_dir)
    print('Done, results in: {}'.format(output_dir))
...
if __name__=='__main__':
    ...
    parser.set_defaults(serial=False, recursive=False, verbose=False)
